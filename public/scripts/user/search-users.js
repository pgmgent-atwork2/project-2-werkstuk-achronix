...
if (document.getElementById("userTableBody")) {
  ...
  $filterSelect.addEventListener("change", async (event) => {
    ...
    try {
      const response = await fetch(`/api/users/role/${filterValue ? filterValue : undefined}`);
      const users = await response.json();
      ...
      console.error("Fout bij het ophalen van gefilterde gebruikers:", error);
      $tableBody.innerHTML =
        "<tr><td colspan='7'>Fout bij het ophalen van gefilterde gebruikers</td></tr>";
    }
  });
}
